...
(function (PruneCluster_1) {
    ...
    var Marker = (function (_super) {
        ...
        Marker.prototype.SetDataProcessor = function (dataProcessor) {
            this.dataProcessor = dataProcessor;
            for (var key in this.data) {
                this.data[key] = data[key];
            }
        };
        Marker.prototype.ProcessData = function () {
            return this.dataProcessor(this.data);
        };
        return Marker;
    })(ClusterObject);
    ...
    var Cluster = (function (_super) {
        ...
        Cluster.prototype.AggregateMarkerData = function () {
            let total = 0;
            this._clusterMarkers.forEach(function (marker) {
                total += marker.ProcessData();
            });
            return total;
        };
    ...
    var PruneCluster = (function () {
        function PruneCluster(markerAgg) {
            this._markers = [];
            this._nbChanges = 0;
            this._clusters = [];
            this.Size = 166;
            ...
            this.markerAggregator = markerAgg;
        }
        ...
        PruneCluster.prototype.ProcessView = function (bounds) {
            ...
            for (var i = firstIndex, l = markers.length; i < l; ++i) {
                ...
                if (markerPosition.lat > extendedBounds.minLat &&
                    ...
                    if (!clusterFound) {
                        cluster = new Cluster(marker, this.markerAggregator);
                        cluster.ComputeBounds(this);
        ...
        // PruneCluster.prototype.AggregateMarkerData = function (markers) {
        //     return this.markerAggregator(markers);
        // };
...
var PruneClusterForLeaflet = (L.Layer ? L.Layer : L.Class).extend({
    initialize: function (size, clusterMargin, markerAggregator) {
        var _this = this;
        if (size === void 0) {
            size = 120;
        }
        ...
        this.Cluster = new PruneCluster.PruneCluster(markerAggregator);
    ...
    BuildLeafletClusterIcon: function (cluster) {
        var c = 'prunecluster prunecluster-intensity-';
        var iconSize = 38;

        var maxPopulation = this.Cluster.GetPopulation();

        var n = Number.parseFloat((cluster.AggregateMarkerData() / cluster.population).toFixed(3));
        let maxIntensity = 15;
        c += (n * maxIntensity).toFixed(0);
    ...
    ProcessView: function () {
        ...
        }, _this.Cluster.markerAggregator);
